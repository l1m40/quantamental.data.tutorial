...
Os índices da B3 agrupam empresas segundo diferentes critérios e atribui um percentual de participação para cada constituinte. A partir dos índices é possível fazer novas composições. A tabela abaixo mostra quantos ativos estão presentes em alguns índices disponíveis na ontologia do projeto.
...
- identifica pares de empresas comparáveis mesmo quando pertencem a setores diferentes;
- encontra relações que não aparecem em classificações econômicas tradicionais;
- produz grupos mais homogêneos para análises quantitativas;
- melhora processos como seleção de comparáveis, construção de fatores, análise de risco e diversificação de portfólio.
...
Até este ponto, vimos como a ontologia organiza o universo de ativos e como a matriz de similaridade permite identificar relações entre empresas e setores. O próximo passo é compreender como cada empresa é representada dentro de um modelo Quantamental.

Uma **feature** é uma característica mensurável de uma empresa. Ela representa um aspecto específico do negócio, do mercado ou do comportamento do ativo e é utilizada como informação de entrada para análises quantitativas. Exemplos de features incluem indicadores fundamentalistas, como Preço/Lucro (P/L), Retorno sobre Patrimônio (ROE) e Margem EBITDA, além de métricas de mercado, como volatilidade, liquidez, retorno acumulado e beta.
...
Essa visão multidimensional é um dos pilares da abordagem Quantamental. Em vez de basear uma decisão em um único indicador, os modelos combinam informações provenientes de diferentes famílias de variáveis para construir uma representação mais completa e objetiva de cada empresa.

Nos próximos capítulos exploraremos essas famílias em maior profundidade, começando pelos indicadores de **Valuation**, utilizando o **Preço/Lucro (P/L)** como exemplo para apresentar conceitos fundamentais da análise Quantamental.
:::
